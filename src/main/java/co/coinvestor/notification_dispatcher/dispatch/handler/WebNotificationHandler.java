...
import co.coinvestor.notification_dispatcher.dispatch.factory.NotificationFactory;
...
public class WebNotificationHandler implements NotificationHandler{
    ...
    private final NotificationFactory webNotificationFactory;
    private final NotificationRepository notificationRepository;
    private final RabbitProducer rabbitProducer;

    @Override
    public boolean able(NSRequestDto requestDto) {
        return requestDto.getDestination().equals(NSRequestDto.Destination.WEB_NOTIFICATION);
    }

    @Override
    public void handle(NSRequestDto requestDto) {
        //0. receiverList size 만큼 message split
        List<Notification> notifications = webNotificationFactory.generateNotifications(requestDto);
